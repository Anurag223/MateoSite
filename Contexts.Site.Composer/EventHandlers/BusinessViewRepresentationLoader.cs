...
{
    [BulkRepresentationLoader("MasterData.BusinessView")]
    [UsedImplicitly]
    public class BusinessViewRepresentationLoader : IBulkRepresentationLoader
    {
        ...
        public BusinessViewRepresentationLoader(IRepository<WorkCenterSite> workCenterSiteCanonicalRepo, IRepresentationBulkComposerAndSaver<SiteRepresentation> siteBulkComposerAndSaver)
        {
        ...
        private async Task<IEnumerable<string>> GetWorkCenterSiteKeysByBusinessTeam(IEnumerable<string> businessViewKeys)
        {
            if (!businessViewKeys.Any())
                return Enumerable.Empty<string>();

            var workCenterSites = await FindWorkCenterSiteBySegmentCode(businessViewKeys);
            return workCenterSites?.Collection.Select(x => x.Id) ?? Enumerable.Empty<string>();
        }

        private Task<CollectionResult<WorkCenterSite>> FindWorkCenterSiteBySegmentCode(IEnumerable<string> businessViewKeys)
        {
            var querySpec = QuerySpec.ByValues(nameof(WorkCenterSite.SegmentCode), businessViewKeys);
