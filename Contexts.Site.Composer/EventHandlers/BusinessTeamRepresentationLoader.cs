...
{
    [BulkRepresentationLoader("MasterData.BusinessTeam")]
    [UsedImplicitly]
    public class BusinessTeamRepresentationLoader : IBulkRepresentationLoader
    {
        ...
        public BusinessTeamRepresentationLoader(IRepository<WorkCenterSite> workCenterSiteCanonicalRepo, IRepresentationBulkComposerAndSaver<SiteRepresentation> siteBulkComposerAndSaver)
        {
        ...
        private async Task<IEnumerable<string>> GetWorkCenterSiteKeysByBusinessTeam(IEnumerable<string> businessTeamKeys)
        {
            if (!businessTeamKeys.Any())
                return Enumerable.Empty<string>();

            var workCenterSites = await FindWorkCenterSiteByGeoMarket(businessTeamKeys);
            return workCenterSites?.Collection.Select(x => x.Id) ?? Enumerable.Empty<string>();
        }

        private Task<CollectionResult<WorkCenterSite>> FindWorkCenterSiteByGeoMarket(IEnumerable<string> businessTeamKeys)
        {
            var querySpec = QuerySpec.ByValues(nameof(WorkCenterSite.GeoMarketCode), businessTeamKeys);
