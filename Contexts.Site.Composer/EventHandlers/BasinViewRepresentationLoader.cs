...
{
    [BulkRepresentationLoader("MasterData.BasinView")]
    [UsedImplicitly]
    public class BasinViewRepresentationLoader : IBulkRepresentationLoader
    {
        ...
        public BasinViewRepresentationLoader(IRepository<WorkCenterSite> workCenterSiteCanonicalRepo, IRepresentationBulkComposerAndSaver<SiteRepresentation> siteBulkComposerAndSaver)
        {
        ...
        private async Task<IEnumerable<string>> GetWorkCenterSiteKeysByBusinessTeam(IEnumerable<string> BasinViewKeys)
        {
            if (!BasinViewKeys.Any())
                return Enumerable.Empty<string>();

            var workCenterSites = await FindWorkCenterSiteBySegmentCode(BasinViewKeys);
            return workCenterSites?.Collection.Select(x => x.Id) ?? Enumerable.Empty<string>();
        }

        private Task<CollectionResult<WorkCenterSite>> FindWorkCenterSiteBySegmentCode(IEnumerable<string> BasinViewKeys)
        {
            var querySpec = QuerySpec.ByValues(nameof(WorkCenterSite.SegmentCode), BasinViewKeys);
